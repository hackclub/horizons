...
| Transaction Detail | `/admin/transactions/{id}` | One transaction with full buyer identity (address/phone), item/event details, and fulfill/unfulfill/refund actions. Shows the user's current balance (red warning banner when negative) and their full admin balance-adjustment history (hours, reason, date, reversed state). The user card ends with the editable user notes card (shared `user.adminComment`, loaded from `GET /api/reviewer/users/{id}/notes`) |
