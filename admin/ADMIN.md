...
| Project Detail | `/admin/projects/{id}` | Viewer + overall editor for one project, in the review dash's visual language (rv tokens, top bar + three-panel layout). Left rail: owner card (click-to-copy email/Slack ID/Hackatime account, Slack DM link, collapsible birthday/address, sus toggle), hours summary with recalculate and Billy date range, project/user notes (same reviewer-notes endpoints the review dash uses), submission list. Center tabs: Overview (screenshot, description, external links, linked Hackatime projects, Joe fraud panel with trust score/justification/deep link, internal admin comment + hours justification), Submission (read-only detail of the selected submission with a "Review this submission" deep link — verdicts are deliberately not editable here), Timeline (lazy-loaded). Right rail: superadmin-gated editor (metadata, URLs, approved hours, lock, perm-reject, Hackatime project linking) and danger zone (unlock, Joe reset & requeue, delete) |
...
| Users | `/admin/users` | Server-paginated user list (50/page, debounced server-side search + sort), hour balance chip per user (spendable balance + earned/spent breakdown, kept in sync after superadmin hours adjustments), Slack ID editing, fraud/sus flag toggles |
