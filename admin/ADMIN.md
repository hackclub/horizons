...
| Transactions | `/admin/transactions` | Unified ledger of shop purchases, event tickets, and admin adjustments — flat chronological view and a by-user fulfilment view with per-user bulk fulfill/unfulfill/refund. Users with a negative balance carry a red ⚠ badge (flat rows, by-user headers, and a Balance stat per group); local balances update as refunds are applied. Shop rows show order notes under the description. Flat rows and by-user headers show a "Ships to" line when the address recipient name differs from the account name. CSV export of the current result set (includes `ship_first_name`/`ship_last_name` from the current address name, and `order_notes`) |
| Transaction Detail | `/admin/transactions/{id}` | One transaction with full buyer identity (address/phone), order notes, item/event details, and fulfill/unfulfill/refund actions. The address block starts with the recipient name from the user's primary HCA address (the account name, marked as such, until it syncs at their next login); orders don't store an address, so this is always the current one. Shows the user's current balance (red warning banner when negative) and their full admin balance-adjustment history (hours, reason, date, reversed state). The user card ends with the editable user notes card (shared `user.adminComment`, loaded from `GET /api/reviewer/users/{id}/notes`) |
