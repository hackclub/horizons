...
| **Users** |||
| GET | `/users` | Admin | Paginated users (`page`, `limit` ≤200, `q` search on name/email/Slack, `sort` incl. decay-aware streak sorts); each user's projects carry only the latest submission; each user carries hour balance fields (`totalApprovedHours`, `totalSpent`, `balance`) computed via per-page grouped aggregates matching `BalanceService` |
