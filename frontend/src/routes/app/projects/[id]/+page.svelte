<script lang="ts">
	...
	let submittedHours = $derived(
		isApproved
			? (latestSubmission?.approvedHours ??
					latestSubmission?.hackatimeHours ??
					null)
			: isPending
...
<div class="relative size-full">
	...
	{:else if error}
		...
		>
			...
			<p
				class="font-bricolage md:text-[32px] text-[24px] font-semibold text-black m-0"
			>
				{error}
			</p>
		</div>
	{:else if project}
		...
		<div
			class="absolute bottom-20 left-1/2 -translate-x-[calc(50%+0.5px)] z-2"
		>
			<div
				class="detail-card bg-[#f3e8d8] border-4 border-black rounded-[20px] p-7.5 shadow-[4px_4px_0px_0px_black] flex flex-col items-start gap-8 overflow-hidden"
				class:exiting={navigating}
				style="width: {getCardWidth()};"
			>
				<div
					class="flex flex-col gap-2 w-full leading-normal text-black"
				>
					<p
						class="font-cook font-semibold m-0 md:text-[36px] text-[28px]"
					>
						{project.projectTitle}
					</p>
					{#if project.description}
						<p
							class="font-bricolage font-semibold m-0 md:text-[32px] text-[24px]"
						>
							{project.description}
						</p>
					{/if}
					...
					{#if hackatimeInfo}
						<p
							class="font-bricolage font-semibold text-black/60 m-0 md:text-[22px] text-[12px]"
						>
							tracked {currentHours} hrs{submittedHours !== null
								? ` · ${hoursLabel} ${submittedHours} hrs`
								: ""}
						</p>
						{#if hackatimeInfo.hackatimeProjects.length > 0}
							<div class="flex flex-wrap gap-1">
								{#each hackatimeInfo.hackatimeProjects as name}
									<span
										class="bg-black text-[#f3e8d8] font-bricolage text-sm font-semibold px-2 py-0.5 rounded-sm whitespace-nowrap"
									>
										{name}
									</span>
								{/each}
				...
				{#if hasSubmission}
					{@const submissionDate = latestSubmission?.createdAt
						...
						: null}
					{@const daysAgo = submissionDate
						? Math.floor(
								(Date.now() - submissionDate.getTime()) /
									(1000 * 60 * 60 * 24),
							)
						: null}
					{@const formattedDate = submissionDate
						? `${submissionDate.getMonth() + 1}/${submissionDate.getDate()}/${submissionDate.getFullYear()}`
						: ""}
					{@const repoUrl =
						latestSubmission?.repoUrl ?? project?.repoUrl}
					{@const demoUrl =
						latestSubmission?.playableUrl ?? project?.playableUrl}
					{@const reviewerFeedback =
						latestSubmission?.hoursJustification}
					{@const step2Color = isApproved
						? "#ffa936"
						: isRejected
							? "#e05632"
							: isPending
					...
					{@const step2Label = isRejected
						? "Rejected"
						: "Under Review"}

					<div
						class="flex flex-col gap-3 w-full border-4 border-black rounded-[20px] p-4"
					>
						<p
							class="font-cook text-[16px] md:text-[28px] font-semibold text-black m-0"
						>
							SUBMISSION TRACKING
						</p>
						...
						<svg
							viewBox="-2 -2 604 52"
							style="width: 100%; aspect-ratio: 604 / 52; display: block;"
						...
						>
							...
							<g clip-path="url(#progress-clip)">
								<path
									d="M 400 0 H 600 V 48 H 400 L 416 24 Z"
									fill={step3Color}
								/>
								{#if isPending}
									<path
										d="M 200 0 H 400 L 416 24 L 400 48 H 200 L 216 24 Z"
										fill={step2Color}
									>
										<animate
								...
								{/if}
								<path
									d="M 0 0 H 200 L 216 24 L 200 48 H 0 Z"
									fill="#ffa936"
								/>
							</g>
							<rect
						...
						<div class="flex gap-3">
							...
							{#if reviewerFeedback}
								<div
									class="flex-1 bg-[rgba(0,0,0,0.1)] rounded-xl p-4 flex flex-col gap-1 min-w-0"
								>
									<p
										class="font-bricolage text-[15px] font-bold text-black m-0"
									>
										Reviewer Feedback
									</p>
									<p
										class="font-bricolage text-[14px] text-black m-0 line-clamp-4"
									>
										{reviewerFeedback}
									</p>
									<button
							...
							<!-- Links / Date panel -->
							<div
								class="flex-1 bg-[rgba(0,0,0,0.1)] rounded-xl p-4 flex flex-col gap-1.5 min-w-0"
							>
								{#if repoUrl}
									<a
										href={repoUrl}
										...
										class="flex items-center gap-2 font-bricolage text-[15px] text-black"
									>
										<!-- GitHub mark -->
										<svg
										...
										>
											<path
												d="M7 1H11M11 1V5M11 1L5 7M2 3H4M2 3V10H9V8"
												stroke="black"
												stroke-width="1.5"
				...
				<div
					class="flex flex-col sm:flex-row gap-2.5 w-full justify-center items-center"
				>
					<button
						...
						class:selected={nav.usingKeyboard &&
							nav.isSelected(0, 0)}
						class:keyboard={nav.usingKeyboard}
						class:pending={isPending}
						onclick={() =>
							navigateTo(`/app/projects/${projectId}/edit`)}
					...
					<button
						...
						onclick={() =>
							navigateTo(
								`/app/projects/${projectId}/ship/presubmit`,
							)}
	...
	{#if feedbackOpen && latestSubmission?.hoursJustification}
		...
		>
			...
			></button>
			<div
				class="relative max-w-2xl w-full max-h-[80vh] bg-[#f3e8d8] border-4 border-black rounded-[20px] p-6 shadow-[4px_4px_0px_0px_black] flex flex-col gap-3 overflow-hidden"
			>
				<div class="flex items-center justify-between gap-4">
					<p
						class="font-cook text-[28px] font-semibold text-black m-0"
					>
						REVIEWER FEEDBACK
					</p>
					<button
				...
				<p
					class="font-bricolage text-[16px] text-black m-0 whitespace-pre-wrap break-words flex-1 min-h-0 overflow-y-auto"
				>
					{latestSubmission.hoursJustification}
				</p>
			</div>
